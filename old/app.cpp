...
void appData::init() {
  ...
  if (gEServer) {
...

    {
      ...
      //while (true) {
        tmp = scanBytes((unsigned char *)tmp, (gEServerSz + gEServer) - tmp,
                        (char *)"6E61746E6567312E67616D657370792E636F6D");
        if (tmp) {
          unprotectMem(tmp);
          memcpy(tmp, "ns01.freenom.com", sizeof("ns01.freenom.com"));
        }
        tmp = scanBytes((unsigned char *)tmp, (gEServerSz + gEServer) - tmp,
                        (char *)"6E61746E6567322E67616D657370792E636F6D");
        if (tmp) {
          unprotectMem(tmp);
          memcpy(tmp, "ns02.freenom.com", sizeof("ns02.freenom.com"));
        }
    }
    {
      unsigned char *tmp =
      ...
      unprotectMem(tmp);
      memcpy(tmp, "gamefear.tk", sizeof("gamefear.tk"));
    }
    /*{
    ...
    }*/
    {
      ...
      unprotectMem(tmp);
      memcpy(tmp, "%s.available.gamefear.tk", sizeof("%s.available.gamefear.tk"));
    }
    {
      unsigned char *tmp = scanBytes(
          (unsigned char *)gEServer, gEServerSz,
