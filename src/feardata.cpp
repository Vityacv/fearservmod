...
void fearData::getLTServerClient(unsigned char *baseMod, uintptr_t szMod) {
  {
    ...
    for (unsigned i = 0; i != 2; i++) {
      tmp=scanBytes2(
                         ...
                         BYTES_SEARCH_FORMAT("6A6968????????68????????E8????????68????????E8????????83C410"));
      if (!tmp) break;
  ...
  if (!g_pLTServer) {
    unsigned char *tmp = 0;
    tmp = scanBytes2((unsigned char *)baseMod, szMod,
...
void fearData::fearDataInitServ() {
  ...
  {
    unsigned char *tmp = 0;
    tmp = scanBytes2((unsigned char *)gServer, gServerSz,
  ...
  {
    void *tmp = scanBytes2((unsigned char *)gServer, gServerSz,
  ...
  }
  {
    ...
    tmp =
        scanBytes2((unsigned char *)gServer, gServerSz,
                  BYTES_SEARCH_FORMAT("8B0D??????????6A00E8????????84C075??85??74??8B??8B"));
    if (tmp) {
      g_pServerMissionMgrAdr = (CServerMissionMgr **)*(uintptr_t *)(tmp + 2);
    }
  }
  g_pGameDatabaseMgr = (CGameDatabaseMgr *)*(uintptr_t *)*(
      uintptr_t *)(scanBytes2((unsigned char *)gServer, gServerSz,
                             BYTES_SEARCH_FORMAT("8B0D??????????E8????????85C074??80380074?"
                                     "???8D????85")) +
                   2);
  {
    unsigned char *tmp =
  ...
  {
    unsigned char *tmp = scanBytes2((unsigned char *)gServer, gServerSz,
                                   BYTES_SEARCH_FORMAT("E8????????8B??????83????0F??????????FF????????????8B"));
...

  {
        unsigned char *tmp =
        scanBytes2((unsigned char *)gServer, gServerSz,
                  BYTES_SEARCH_FORMAT("D986????????D94424??D84C24??D94424??D84C24??DEC1D94424??D84C24??DEC1D9C1"));
  ...
  }
  {
        unsigned char *tmp =
        scanBytes2((unsigned char *)gServer, gServerSz,
                  BYTES_SEARCH_FORMAT("D986????????D94424??D84C24??D94424??D84C24??DEC1D94424??D84C24??DEC1D9FA"));
  ...
  {
    unsigned char * tmp=scanBytes2((unsigned char *)gServer, gServerSz,
                        BYTES_SEARCH_FORMAT("8B????????????89??????????E8????????8B??E8????????85C00F??????????3A??????????0F??????????8A??????88"));
    if (tmp) {
  ...
  {
    unsigned char * tmp=scanBytes2((unsigned char *)gServer, gServerSz,
                        BYTES_SEARCH_FORMAT("89??????????89??????????89??????????8B??????????E8????????84C00F??????????8D????????8D??????????E8"));
    if (tmp) {
  ...
  {
    unsigned char * tmp=scanBytes2((unsigned char *)gServer, gServerSz,
                        BYTES_SEARCH_FORMAT("C6??????????01EB0F68????????6A01E8????????83C4088B??????8B??????8B??????89"));
    if (tmp) {
  ...
  }
  {
    unsigned char *tmp = scanBytes2(
        ...
        BYTES_SEARCH_FORMAT("D9??????????D8??????????8B??6A208B??D9??????8B442414"));
    if (tmp) {
  ...
  }
  {
    unsigned char *tmp = scanBytes2(
        (unsigned char *)gServer, gServerSz,
        BYTES_SEARCH_FORMAT("83EC????8B??8B??????????85??0F??????????8B??????????8B"));
    if (tmp) {
  ...
  }
  {
    unsigned char *tmp =
        scanBytes2((unsigned char *)gServer, gServerSz,
                  BYTES_SEARCH_FORMAT("E8????????8B??????????8B??????8B??89??????6A01"));
...

  {
    unsigned char *tmp =
        scanBytes2((unsigned char *)gServer, gServerSz,
                  BYTES_SEARCH_FORMAT("8D??????????E8????????8B??????????3B??74??6A01"));
...

  {
    unsigned char *tmp =
        scanBytes2((unsigned char *)gServer, gServerSz,
                  BYTES_SEARCH_FORMAT("FF52??89??????8B????????6A00??8B??????????E8"));
  ...
  {
    unsigned char * tmp = scanBytes2((unsigned char *)gServer, gServerSz,
                             BYTES_SEARCH_FORMAT("E8????????8B??????????6A006A0068??????????89??????E8????????83"));
...
void fearData::fearDataInit() {
  ...
  {
    void *tmp = scanBytes2((unsigned char *)gClient, gClientSz,
...

  {
    void *tmp = scanBytes2(
        ...
        BYTES_SEARCH_FORMAT("8B0D????????5053E8????????8AD88B44240CF6DB1ADB8D4C2410"));
    g_pClientConnectionMgr =
        (ClientConnectionMgr *)*(uintptr_t *)((unsigned char *)tmp + 2);
  }
  {
    unsigned char *tmp = (unsigned char *)(uintptr_t *)(scanBytes2(
        (unsigned char *)gClient, gClientSz,
        BYTES_SEARCH_FORMAT("892D????????E8????????A1????????508D8D????????E8????????"
                "33DBC685????????01")));
    if (tmp) g_pScreenMultiAdr = tmp + 2;
  }
  spliceUp(scanBytes2((unsigned char *)gClient, gClientSz,
                     BYTES_SEARCH_FORMAT("8B??68????????E8????????8B????8B??????????8B??33?"
                             "?????68??????????FF")),
  ...
  if (doConnectIpAdrTramp) {
    {
      void *tmp = scanBytes2((unsigned char *)gFearExe, gFearExeSz,
    ...
    getStructFromString1 = scanBytes2((unsigned char *)gFearExe, gFearExeSz,
                                     BYTES_SEARCH_FORMAT("5355568BE98B45145733FF85C07638"));

    {
      void *tmp =
          scanBytes2((unsigned char *)gFearExe, gFearExeSz,
                    BYTES_SEARCH_FORMAT("8B0D????????50E8????????85C08B4C24108901740C"));
  ...
  g_pGameDatabaseMgr = (CGameDatabaseMgr *)*(uintptr_t *)*(
      uintptr_t *)(scanBytes2((unsigned char *)gClient, gClientSz,
                             BYTES_SEARCH_FORMAT("8B0D????????8B41248B1650A1????????"
