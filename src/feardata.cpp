...
HWEAPONDATA fearData::GetWeaponData(HWEAPON hWep) {
  return ((HWEAPONDATA(__thiscall *)(void *, HWEAPON, bool))
              g_pWeaponDB_GetWeaponData)(g_pWeaponDB, hWep, !USE_AI_DATA);
}

HWEAPONDATA fearData::GetAmmoData(HAMMO hAmmo) {
  return ((HWEAPONDATA(__thiscall *)(void *, HWEAPON, bool))
              g_pWeaponDB_GetAmmoData)(g_pWeaponDB, hAmmo, !USE_AI_DATA);
}
...
void fearData::addInternetServer(char *str) {
  unsigned char *g_pScreenMulti =
      (unsigned char *)*(uintptr_t *)*(uintptr_t *)g_pScreenMultiAdr;
  ((void(__thiscall *)(void *, char *, unsigned, unsigned))
       g_pScreenMulti_RequestServerDetails)(g_pScreenMulti, str, 1, 1);
}
...
template <typename T> void shuffle(T *array, size_t n) {
  if (n > 1) {
    size_t i;
    for (i = 0; i < n - 1; i++) {
...
void regcall fearData::hookOnMapLoaded(reg *p) {
  ...
  ((void(__thiscall *)(void *))pSdk->g_pServerVoteMgr_ClearVote)(
      pSdk->g_pServerVoteMgr);
  if (aData->bCoop) {
